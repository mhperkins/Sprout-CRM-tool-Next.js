...
| `relationship_types`        | string[]         | Multi-select. Enum: `music` `art` `event_host` `partner` `community_builder` `attendee` `coworking` `showcase` `sprout_society` `other`. Legacy single-string coerced to array on read. |
