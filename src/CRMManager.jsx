...
/* ─── Storage ──────────────────────────────────────────────────────────────── */
const CK = "sprout_crm_";
const db = {
  get: (k) => { try { const v = localStorage.getItem(CK + k); return v ? JSON.parse(v) : null; } catch { return null; } },
  set: (k, v) => { try { localStorage.setItem(CK + k, JSON.stringify(v)); } catch {} },
};
...
const SEED_ORGS = [{
  ...
  confidence:"HIGH", createdAt:new Date().toISOString(),
}];
...
/* ─── Contacts View ──────────────────────────────────────────────────────────── */
function ContactsView({contacts,orgs,onUpdate,onDelete,showToast}) {
    ...
    <div className="page">
      ...
      {filtered.length===0
        ...
        : <div className="tbl-wrap"><table className="tbl"><tbody>
            {filtered.map(c=>{<thead><tr><th>Name</th><th>Type</th><th>Organization</th><th>Status</th><th>Tier</th><th>Next Action</th><th>Health</th><th></th></tr></thead>
              const org=orgs.find(o=>o.id===c.org_id); const score=healthScore(c); const od=isOverdue(c);
              return <tr key={c.id} onClick={()=>setSelected(c.id)}>
                ...
                <td><button className="btn btn-danger btn-xs" onClick={e=>{e.stopPropagation();if(window.confirm(`Delete ${c.first_name} ${c.last_name}?`))onDelete(c.id);}}>Del</button></td>
              </tr>;
            })}
          </tbody></table></div>
      }
...
function OutreachView({contacts,orgs}) {
  ...
  const all=[];
  contacts.forEach(c=>(c.touchpoints||[]).forEach(tp=>all.push({...tp,personName:`${c.first_name} ${c.last_name}`,orgName:orgs.find(o=>o.id===c.org_id)?.name||""})));
  orgs.forEach(o=>(o.touchpoints||[]).forEach(tp=>all.push({...tp,personName:o.name,orgName:"Organization record"})));
...
export default function CRMApp() {
  ...
  useEffect(()=>{
    const c=db.get("contacts"); const o=db.get("orgs"); const p=db.get("posts"); const pr=db.get("profile");
    setContacts(c||SEED_CONTACTS); setOrgs(o||SEED_ORGS); setPosts(p||[]); if (pr) setProfile(pr);
    setLoading(false);
  },[]);

  const showToast=useCallback((msg,type="ok")=>{ setToast({msg,type}); setTimeout(()=>setToast(null),3200); },[]);
  const saveContacts=useCallback((u)=>{ setContacts(u); db.set("contacts",u); },[]);
  const saveOrgs=useCallback((u)=>{ setOrgs(u); db.set("orgs",u); },[]);
  const savePosts=useCallback((u)=>{ setPosts(u); db.set("posts",u); },[]);
  const saveProfile=useCallback((u)=>{ setProfile(u); db.set("profile",u); },[]);
...
const deleteContact=useCallback((id)=>{ saveContacts(contacts.filter(c=>c.id!==id)); showToast("Contact deleted"); },[contacts,saveContacts,showToast]);
