                ...
                <tr>
                  <td valign="top">
                    ...
                    ${(() => {
                      ...
                      const cell = (b, bg) => `<td style="border-radius:6px; background-color:${bg};"><a href="${esc(b.url)}" target="_blank" rel="noopener noreferrer" style="display:inline-block; padding:10px 22px; font-family:'Lato',Helvetica,Arial,sans-serif; font-size:13px; font-weight:900; letter-spacing:0.5px; color:#ffffff; text-decoration:none; border-radius:6px;">${esc(b.label)} &rarr;</a></td>`;
                      const cells = [donate ? cell(donate, "#E10098") : "", member ? cell(member, "#2a8ca0") : ""].filter(Boolean);
                      if (!cells.length) return "";
                      return `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 16px 0;"><tr>${cells.join('<td style="width:10px;">&nbsp;</td>')}</tr></table>`;
                    })()}
