...
export const COMPACT_SECTIONS = [
  ...
  { key: "upcoming",       label: "Upcoming events",                  kind: "repeat", itemLabel: "event", itemImage: true,
    fields: [{ k: "date", ph: "Date" }, { k: "name", ph: "Event name" }, { k: "note", ph: "One line", polish: true }, { k: "link", ph: "Sign-up / RSVP link (optional)" }] },
                      ...
                      <tr><td style="padding:18px 16px; font-family:'Lato',Helvetica,Arial,sans-serif;">
                        ...
                        <span style="font-size:13px; line-height:1.5; color:#3a3a38;">${cval(e.note, "One line")}</span>${
                          (e.link || "").toString().trim()
                            ? `<br><a href="${esc(e.link)}" target="_blank" rel="noopener noreferrer" style="display:inline-block; margin-top:8px; font-size:13px; font-weight:900; color:#E10098; text-decoration:none;">Sign up / RSVP &rarr;</a>`
                            : ""
                        }
                      </td></tr>
                    </table>
...
export function buildCompact({ fieldValues = {}, monthYear, profile = {}, today } = {}) {
  ...
  const website = profile.website
    ? `<a href="${esc(/^https?:\/\//i.test(profile.website) ? profile.website : `https://${profile.website}`)}" target="_blank" rel="noopener noreferrer" style="${linkStyle}">${esc(profile.website.replace(/^https?:\/\//i, ""))}</a>`
    : cph("Website");
                  ...
                  <td style="padding:22px 22px; font-family:'Lato',Helvetica,Arial,sans-serif;">
                    ...
                    ${
                      (v.spotlightImage || "").toString().trim()
                        ? `<img src="${esc(v.spotlightImage)}" alt="${esc(v.spotlightName || "Member")}" width="64" height="64" style="display:block; width:64px; height:64px; border-radius:50%; object-fit:cover; border:0; margin:6px 0 8px 0;">`
                        : ""
          ...
          <tr>
            <td style="padding:34px 28px; background-color:#030000; font-family:'Lato',Helvetica,Arial,sans-serif;">
              ...
              <p style="margin:0; font-size:11px; line-height:1.6; color:#9a9a96;">
                <a href="mailto:hello@sproutsociety.org?subject=Unsubscribe" style="color:#9a9a96; text-decoration:underline;">Unsubscribe</a> &nbsp;·&nbsp; ${esc(profile.address || "Sprout Society, Brooklyn, NY")}
              </p>
            </td>
          </tr>
