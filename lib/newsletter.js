...
const compactUpCard = (e, side) => {
  ...
  const img = e.image
    ? `<img src="${esc(e.image)}" alt="${esc(e.name || "Event")}" width="100%" style="display:block; width:100%; height:auto; border:0; border-radius:6px; margin-bottom:10px;">`
    : "";
          ...
          <tr>
            <td style="padding:0 28px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#73C4D6" style="border-radius:8px; overflow:hidden;">
                <tr>
                  <td align="center" valign="middle" style="font-family:'Lato',Helvetica,Arial,sans-serif; font-size:13px; font-weight:700; color:#0d3d49;">
                    ${
                      photo
                        ? `<img src="${esc(photo)}" alt="${cval(v.featuredTitle, "Featured event")}" width="544" style="display:block; width:100%; max-width:544px; height:auto; border:0;">`
                        : `<div style="padding:70px 16px;">${cph("Paste a featured photo URL")}</div>`
                    }
                  </td>
                </tr>
              </table>
