...
export const COMPACT_SECTIONS = [
  ...
  { key: "donateLink",     label: "Footer — Donate button (URL, or [Label](URL))",    kind: "single", ph: "https://givebutter.com/sproutspacedonors" },
  { key: "memberLink",     label: "Footer — Become a member button (URL, or [Label](URL))", kind: "single", ph: "https://givebutter.com/sproutmembership" },
  { key: "footerIG",       label: "Footer — Instagram handle",        kind: "header", ph: "@sproutsocietyorg" },
  { key: "footerAddress",  label: "Footer — address",                 kind: "header", ph: "26 Scott Ave, Brooklyn, NY 11237" },
  { key: "footerEIN",      label: "Footer — EIN",                     kind: "header", ph: "83-1298420" },
];

/* ─── Quick Hit (one-off): structured section model ──────────────────────────────
   The one-off uses the SAME structured-section machinery as the compact roundup, so
   it gets the identical editor (text boxes, ✨ Polish, 🖼 image add, drag-crop, link
   parsing, live preview). Just fewer sections: heading, body, announcements, upcoming.
   buildQuickHit() renders HTML from these values. */
// kind: "header" = an editable static title/eyebrow. Blank falls back to its default
// text (`ph`), so leaving it empty keeps the original wording (matches the roundup).
export const QUICK_HIT_SECTIONS = [
  { key: "mastheadLabel", label: "Masthead — label",         kind: "header", ph: "Sprout Society" },
  ...
  { key: "announcements", label: "Announcements",            kind: "repeat", itemLabel: "announcement", itemImage: true, itemCrop: true, itemRatio: 544 / 200,
    fields: [{ k: "eyebrow", ph: "Header (default: Announcement)" }, { k: "title", ph: "Announcement title" }, { k: "body", ph: "One or two lines", polish: true }, { k: "link", ph: "Button link (URL)" }, { k: "linkLabel", ph: "Button label (e.g. Learn more)" }] },
  { key: "upcomingTitle", label: "Upcoming — header",        kind: "header", ph: "Next up" },
  { key: "upcoming",      label: "Upcoming event",           kind: "repeat", itemLabel: "event", itemImage: true, itemCrop: true, itemRatio: 544 / 180,
    fields: [{ k: "date", ph: "Date" }, { k: "name", ph: "Event name" }, { k: "note", ph: "Time · Location · one line", polish: true }, { k: "link", ph: "Button: URL, or [Button text](URL)" }] },
  { key: "footerBrand",   label: "Footer — brand name",      kind: "header", ph: "SPROUT SOCIETY" },
  { key: "donateLink",    label: "Footer — Donate button (URL, or [Label](URL))", kind: "single", ph: "https://givebutter.com/sproutspacedonors" },
  { key: "memberLink",    label: "Footer — Become a member button (URL, or [Label](URL))", kind: "single", ph: "https://givebutter.com/sproutmembership" },
  { key: "footerIG",      label: "Footer — Instagram handle", kind: "header", ph: "@sproutsocietyorg" },
...
/* ─── Reorderable content blocks ────────────────────────────────────────────────
   Which editor fields group into each draggable "section", and the default order.
   Masthead/intro (top) and footer (bottom) stay fixed; only these blocks move.
   The chosen order is stored in field_values.section_order (an array of block ids). */
export const COMPACT_BLOCKS = [
  { id: "featured",    label: "Featured event",           keys: ["featuredEyebrow", "featuredTitle", "featuredRecap", "featuredAnnounce", "featuredPhoto"] },
  { id: "announce",    label: "Announcement",             keys: ["coworkingEyebrow", "coworkingTitle", "coworking", "coworkingChip", "coworkingThurs"] },
  { id: "membership",  label: "Membership",               keys: ["membership", "membershipBtn", "membershipLink"] },
  { id: "marketing",   label: "Marketing",                keys: ["marketing", "marketingLink"] },
  { id: "scholarship", label: "Scholarship",              keys: ["scholarship", "scholarshipBtn", "scholarshipLink"] },
  { id: "spotlight",   label: "Community spotlight",      keys: ["spotlightEyebrow", "spotlightName", "spotlightBlurb", "spotlightImage", "spotlightIG", "spotlightWeb", "spotlightTestimonial"] },
  { id: "events",      label: "Events (upcoming + past)", keys: ["upcomingTitle", "upcoming", "pastTitle", "past"] },
];
export const COMPACT_FIXED_TOP = ["mastheadLabel", "headline", "intro"];
export const COMPACT_FIXED_BOTTOM = ["footerBrand", "donateLink", "memberLink", "footerIG", "footerAddress", "footerEIN"];

export const QUICK_HIT_BLOCKS = [
  { id: "announcements", label: "Announcements", keys: ["announcements"] },
  { id: "upcoming",      label: "Upcoming",      keys: ["upcomingTitle", "upcoming"] },
];
export const QH_FIXED_TOP = ["mastheadLabel", "headline", "body"];
export const QH_FIXED_BOTTOM = ["footerBrand", "donateLink", "memberLink", "footerIG", "footerAddress", "footerEIN"];

// Resolve a saved order against the known blocks: keep saved ids that still exist
// (deduped), then append any blocks the saved order didn't mention. So new blocks
// always appear and a stale/foreign id can never drop or duplicate a section.
export function orderedBlockIds(saved, blocks) {
...
// Like parseBtn but ALWAYS returns a button: a blank field uses the default label + URL,
// a bare URL keeps the default label, `[Label](url)` sets both. Used for the footer
// Donate / Become-a-member buttons so they always render and are fully editable.
const footerBtn = (raw, defLabel, defUrl) => {
  const s = (raw || "").toString().trim();
  if (!s) return { label: defLabel, url: defUrl };
  const m = s.match(/^\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)$/);
  if (m) return { label: m[1], url: m[2] };
  return { label: defLabel, url: s };
};
// Footer defaults shared by both templates.
const FOOTER_DEFAULTS = {
  donateLabel: "Donate",   donateUrl: "https://givebutter.com/sproutspacedonors",
  memberLabel: "Become a member", memberUrl: "https://givebutter.com/sproutmembership",
  ig: "@sproutsocietyorg",
  address: "26 Scott Ave, Brooklyn, NY 11237",
  ein: "83-1298420",
};
// Resolve the footer IG handle: per-newsletter field > org profile > default.
const footerIgParts = (vIG, profile = {}) => {
  const raw = ((vIG || "").toString().trim() || profile.igHandle || FOOTER_DEFAULTS.ig).trim();
  const display = raw.startsWith("@") ? raw : `@${raw}`;
  const url = `https://instagram.com/${display.replace(/^@/, "")}`;
  return { display, url };
};
...
export function buildCompact({ fieldValues = {}, monthYear, profile = {}, today } = {}) {
  ...
  // Reorderable content blocks (see COMPACT_BLOCKS). Masthead/intro (above) and
  // footer (below) stay fixed; these render in field_values.section_order.
  const blocks = {
    ...
    events: `          <!-- EVENTS (scroll target — id on a real row so it isn't foster-parented out of the table) -->
          <tr id="events">
...
${compactPastRows(v.past)}
  ...
  const bodyBlocks = orderedBlockIds(v.section_order, COMPACT_BLOCKS)
    .map((id) => blocks[id] || "")
    .join("\n");
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td valign="top">
                    ...
                    ${(() => {
                      const fig = footerIgParts(v.footerIG, profile);
                      return `<p style="margin:0 0 14px 0; font-size:13px; line-height:1.6; color:#6a6a66;">Brooklyn, NY &nbsp;·&nbsp; <a href="${esc(fig.url)}" target="_blank" rel="noopener noreferrer" style="${linkStyle}">${esc(fig.display)}</a></p>`;
                    })()}
                    ${(() => {
                      const donate = footerBtn(v.donateLink, FOOTER_DEFAULTS.donateLabel, FOOTER_DEFAULTS.donateUrl);
                      const member = footerBtn(v.memberLink, FOOTER_DEFAULTS.memberLabel, FOOTER_DEFAULTS.memberUrl);
                      ...
                      return `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 16px 0;"><tr>${cell(donate, "#E10098")}</tr>${spacer}<tr>${cell(member, "#2a8ca0")}</tr></table>`;
                    })()}
                    <p style="margin:0; font-size:11px; line-height:1.6; color:#8a8a86;">
                      <unsubscribe><span style="color:#8a8a86; text-decoration:underline;">Unsubscribe</span></unsubscribe> &nbsp;·&nbsp; ${esc((v.footerAddress || "").toString().trim() || profile.address || FOOTER_DEFAULTS.address)}<br>Sprout Society is a registered 501(c)(3) nonprofit &nbsp;·&nbsp; EIN ${esc((v.footerEIN || "").toString().trim() || profile.ein || FOOTER_DEFAULTS.ein)}
                    </p>
                  </td>
                ...
                <td style="padding:0 18px 16px 18px;">
                  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#F7F7F6" style="border-radius:8px; overflow:hidden; box-shadow:0 4px 14px rgba(0,0,0,0.16);">
                    <tr>
                      <td width="6" bgcolor="#73C4D6" style="width:6px; line-height:1px; font-size:1px;">&nbsp;</td>
                      <td style="padding:20px 22px; font-family:'Lato',Helvetica,Arial,sans-serif;">
                        ${img}<span style="font-size:12px; font-weight:900; letter-spacing:2px; color:#73C4D6; text-transform:uppercase;">${tval(a.eyebrow, "Announcement")}</span><br>
...
export function buildQuickHit({ fieldValues = {}, monthYear, profile = {}, today } = {}) {
  const v = fieldValues || {};
  const monthLong = monthYear || defaultMonthYear(today) || "Month Year";
  // Reorderable content blocks (see QUICK_HIT_BLOCKS). Masthead/heading/body (above)
  // and footer (below) stay fixed; these render in field_values.section_order.
  const blocks = {
    announcements: `          <!-- ANNOUNCEMENTS -->
${qhAnnounceRows(v.announcements)}`,
    upcoming: `          <!-- UPCOMING -->
          <tr>
            <td data-sec="upcomingTitle" style="padding:6px 18px 4px 18px; font-family:'Lato',Helvetica,Arial,sans-serif; font-size:12px; font-weight:900; letter-spacing:2px; color:#E10098; text-transform:uppercase;">
              ${tval(v.upcomingTitle, "Next up")}
            </td>
          </tr>
          <tr>
            <td data-sec="upcoming" style="padding:0 18px 6px 18px;">
...
${compactUpcomingRows(v.upcoming)}
  ...
  const bodyBlocks = orderedBlockIds(v.section_order, QUICK_HIT_BLOCKS)
    .map((id) => blocks[id] || "")
    .join("\n");
  return `<!DOCTYPE html>
<html lang="en">
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%; max-width:600px; background-color:#FFFFFF; border-radius:8px; overflow:hidden;">
            ...
            <td style="padding:30px 18px 20px 18px; background-color:#FFFFFF;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  ...
                  <td data-sec="mastheadLabel" align="right" valign="middle" style="font-family:'Lato',Helvetica,Arial,sans-serif; font-size:11px; font-weight:700; letter-spacing:2px; color:#030000; text-transform:uppercase;">
                    ${tval(v.mastheadLabel, "Sprout Society")}<br>
                    <span style="color:#8a8a86;">${esc(monthLong)}</span>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td data-sec="body" style="padding:10px 18px 22px 18px; font-family:'Lato',Helvetica,Arial,sans-serif; font-size:16px; line-height:1.6; color:#3a3a38;">
              ${cval(v.body, "A few sentences — recap, news, a thank-you.")}
            </td>
          </tr>
...
${bodyBlocks}
          ...
          <!-- FOOTER -->
          <tr>
            <td data-sec="footerBrand" style="padding:34px 20px; background-color:#FFFFFF; border-top:1px solid #ECECEA; font-family:'Lato',Helvetica,Arial,sans-serif;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td valign="top">
                    <p style="margin:0 0 6px 0; font-size:14px; font-weight:900; color:#030000; letter-spacing:1px;">${tval(v.footerBrand, "SPROUT SOCIETY")}</p>
                    ${(() => {
                      const fig = footerIgParts(v.footerIG, profile);
                      return `<p style="margin:0 0 14px 0; font-size:13px; line-height:1.6; color:#6a6a66;">Brooklyn, NY &nbsp;·&nbsp; <a href="${esc(fig.url)}" target="_blank" rel="noopener noreferrer" style="color:#6a6a66; text-decoration:underline;">${esc(fig.display)}</a></p>`;
                    })()}
                    ${(() => {
                      const donate = footerBtn(v.donateLink, FOOTER_DEFAULTS.donateLabel, FOOTER_DEFAULTS.donateUrl);
                      const member = footerBtn(v.memberLink, FOOTER_DEFAULTS.memberLabel, FOOTER_DEFAULTS.memberUrl);
