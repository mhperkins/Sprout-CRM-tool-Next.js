...
export async function saveContacts(contacts) {
  ...
  for (const c of contacts) {
    const { data, error } = validateContact(c);
    if (error) { console.warn("saveContacts — invalid record skipped:", c.id, error); continue; }
rows.push({
      id:                  data.id,
...
export async function saveOrgs(orgs) {
  const rows = [];
  for (const o of orgs) {
    const { data, error } = validateOrg(o);
    if (error) { console.warn("saveOrgs — invalid record skipped:", o.id, error); continue; }
    rows.push({
      id:                  data.id,
      name:                data.name,
