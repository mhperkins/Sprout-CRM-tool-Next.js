...
/** Stamp when staff were last told about activity on this portal (alert throttle). */
export async function markPortalAlerted(portalId, whenISO = new Date().toISOString()) {
  const { error } = await svc()
    .from("sprout_event_portals")
    .update({ alerted_at: whenISO })
    .eq("id", portalId);
  if (error) console.error("markPortalAlerted failed:", error.message);
}
