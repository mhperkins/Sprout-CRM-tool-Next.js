...
const TouchpointSchema = z.object({
  ...
  outcome:   z.string().nullish().transform(v => v ?? ""),
  next_step: z.string().nullish().transform(v => v ?? ""),
});
...
export const ContactSchema = z.object({
  ...
  relationship_status: z.enum(["cold", "warm", "active", "lapsed", "declined"]).nullish().transform(v => v ?? "cold"),
  tier:                z.enum(["A", "B", "C"]).nullish().transform(v => v ?? "B"),
  ...
  confidence:           z.enum(["HIGH", "MEDIUM", "LOW"]).nullish().transform(v => v ?? "MEDIUM"),
  ask_readiness:        z.enum(["not_ready", "cultivating", "ready", "asked"]).nullish().transform(v => v ?? "not_ready"),
